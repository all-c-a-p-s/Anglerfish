...
The `interactor` is a program that allows the engine to play a match against itself, a human opponent, or another program implementing the same protocol. It is written in Haskell because I need to try and learn Haskell for uni. It's also the first thing I've ever made in Haskell so the code is probably awful.
